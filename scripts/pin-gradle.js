...
async function fetchRemotePins() {
	...
	try {
		...
		// FIX: Changed const to let to prevent TypeError on reassignment
		let out = {};
...
(async () => {
	...
	if (!PINNED) {
		...
		return; // FIX: Replaced process.exit(0) to allow graceful teardown
	}
	...
	if (!fs.existsSync(WRAPPER)) {
		...
		return; // FIX: Replaced process.exit(0)
	}
	...
	console.log(
		`[pin-gradle] pinned Gradle wrapper to ${PINNED}${remote && sdk in remote ? " (from remote manifest)" : ""}`,
	);
})();
