...
export default function Kuru({ showToast }) {
	...
	function hesapla() {
		...
		const OnePointEight = (1.8 * gerilimValue) / SQRT3;
		const OnePointThree = (1.3 * gerilimValue) / SQRT3;

		setResult([
			{
				birnoktasekiz: formatVoltAmp(OnePointEight),
				birnoktauc: formatVoltAmp(OnePointThree),
			},
		]);
	}
	...
	const renderItem = ({ item }) => {
		const fields = [
			{ label: "1.8", value: item.birnoktasekiz + " V" },
			{ label: "1.3", value: item.birnoktauc + " V" },
		];
